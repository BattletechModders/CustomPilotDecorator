...
namespace Gif.Components {
  public class GifDecoder {
    ...
    public class GifFrame {
      ...
      private Texture2D f_texture = null;
      public Texture2D texture {
        get {
          if((f_texture == null) && (image != null)) {
            Log.WL(1,"getting texture image:"+image.Width+"x"+image.Height,true);
            MemoryStream tempStream = new MemoryStream();
            f_texture = new Texture2D(image.Width, image.Height);
            Log.WL(1, "image saving "+ ImageFormat.Bmp.ToString() + " "+ ImageFormat.Bmp.Guid, true);
            try {
              image.Save(tempStream, ImageFormat.Bmp);
            }catch(Exception e) {
              Log.TWL(0,e.ToString(),true);
            }
            Log.WL(1, "image saved", true);
            f_texture.LoadImage(tempStream.ToArray());
            Log.WL(1, "image loaded", true);
            image = null;
            f_texture.Apply();
            Log.WL(1, "image applied", true);
          }
          return f_texture;
        }
      }
    ...
    int[] GetPixels(Bitmap bitmap) {
      ...
      for (int th = 0; th < image.Height; th++) {
        for (int tw = 0; tw < image.Width; tw++) {
          System.Drawing.Color color = bitmap.GetPixel(tw, th);
    ...
    void SetPixels(int[] pixels) {
      ...
      for (int th = 0; th < image.Height; th++) {
        for (int tw = 0; tw < image.Width; tw++) {
          System.Drawing.Color color = System.Drawing.Color.FromArgb(pixels[count++]);
      ...
      if (lastDispose > 0) {
        ...
        if (lastImage != null) {
          ...
          if (lastDispose == 2) {
            ...
            if (transparency) {
              c = System.Drawing.Color.FromArgb(0, 0, 0, 0);   // assume background is transparent
            } else {
              c = System.Drawing.Color.FromArgb(lastBgColor);
    ...
    public Texture2D GetTexture(int n) {
      Texture2D im = null;
      if ((n >= 0) && (n < frameCount)) {
        im = ((GifFrame)frames[n]).texture;
      }
      return im;
    }
